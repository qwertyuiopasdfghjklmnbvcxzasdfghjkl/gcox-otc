  ...
  <div class="order-list">
    ...
    <div class="order-list-content" v-if="getApiToken">
        ...
        <ul class="uncompleted-list">
          <li class="uncompleted-item" v-for="item in data1" :key="item.order_id">
            ...
            <div class="uncompleted-title">
              <span class="numer">{{$t('otc_exchange.otc_exchange_order_number')}}：
              ...
              <span class="last">
                        <template v-if="item.state === 1">
                          ...
                          <template v-if="item.pay_state === 1">
                            <em @click="apeal(item)" v-if="item.appeal_state !== 0">{{$t('otc_exchange.otc_exchange_complaint')}}
                              <!--发起申诉--></em>
      ...
      <div class="completed" v-if="params.state === 2">
        ...
        <page v-if="!loading2 && data2.length > 0" :pageIndex="params.page" :pageSize="params.show"
              :total="params.total" @changePageIndex="pageChange"/>
      ...
      <div class="cancelled" v-if="params.state === 3">
        ...
        <page v-if="!loading3 && data3.length > 0" :pageIndex="params.page" :pageSize="params.show"
              :total="params.total" @changePageIndex="pageChange"/>
...
<style scoped>
  ...
  .uncompleted-title span.last {
    justify-content: flex-end;
    width: 230px;
    margin-left: 18px;
  }

  .uncompleted-title span em {
    color: #00a0e9;
    padding-left: 10px;
    cursor: pointer;
  }

  .uncompleted-title span em:hover {
    color: #2565c5;
  }

  .uncompleted-title span em.disabled {
    color: #999;
    cursor: not-allowed;
  }

  .uncompleted-title span.second em {
    height: 24px;
    line-height: 24px;
  }
  ...
  .uncompleted-title span.last i {
    color: #F34246;
  }

  .uncompleted-title span.last i.time-text {
    text-indent: 6px;
  }
  ...
  .cancelled-item span.type {
    width: 60px;
  }

  .completed-item span.stauts,
  .cancelled-item span.stauts {
    width: 80px;
  }

  .completed-item span.curreny,
  .cancelled-item span.curreny {
    width: 60px;
  }

  .completed-item span.price,
  .cancelled-item span.price {
    width: 130px;
  }

  .completed-item span.number,
  .cancelled-item span.number {
    width: 160px;
  }

  .completed-item span.money,
  .cancelled-item span.money {
    width: 130px;
  }
  ...
  .completed-item span.action .action-botton {
    height: 24px;
    padding: 0 8px;
    line-height: 24px;
    text-align: center;
  ...
  .completed-item span.action .action-botton:hover {
    color: #2565c5;
    border-color: #2565c5;
  }

  .completed-item span.action em.icon-chat {
    font-size: 20px;
    color: #00a0e9;
    vertical-align: middle;
  }

  .completed-item span.action em.icon-chat:hover {
    color: #2565c5;
  }
