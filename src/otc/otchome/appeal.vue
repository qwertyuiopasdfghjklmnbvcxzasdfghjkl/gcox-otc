  ...
  <div class="appeal">
    ...
    <div class="appeal-cont">
      ...
      <form class="question" ref="appealform" onsubmit="return false">
        <div class="question-row question-select">
          ...
          <select :class="{error: errors.has('type')}" name="type" v-model="formData.type" v-validate="'required'">
            <option v-for="(appeal) in appealTtpe" :key="appeal.appeal_type_id" :value="appeal.appeal_type_id">
              {{$t(`otc_exchange.otc_exchange_${appeal.type_key}`)}}
            </option>
          </select>
        ...
        <div class="question-row" v-if="currentTypeKey">
          ...
          <textarea :class="{error: errors.has('description')}" name="description" v-model="formData.description"
                    v-validate="currentTypeKey === 'OTHER' ? 'required' : null"
                    :placeholder="currentTypeKey === 'OTHER' ? $t('public0.public236') : $t('public0.public237')"
        ...
        <div class="question-row question-button">
          ...
          <input class="confirm" type="button" :value="$t('otc_exchange.otc_exchange_determine')" @click="appealSubmit">
          <!--确定-->
        </div>
      </form>
    </div>
...
<style scoped>
  ...
  .question-button input.cancel:hover {
    color: #fdb902;
    border-color: #fdb902;
  }

  .question-button input.confirm {
    color: #ffffff;
    background-color: #fdb902;
  }

  .question-button input.confirm:hover {
    background-color: #fdb902;
  }
</style>
