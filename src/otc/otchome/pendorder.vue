<template>
  <div class="pendorder">
    <div class="pendorder-title">
      ...
      <span class="operate">
            <a class="notice" href="javascript:;" @click="isShowNotice = !isShowNotice">{{$t('otc_ad.otc_trade_tips')}}
              <!--交易须知--></a>
    ...
    <div class="notice-dialog" v-show="isShowNotice">
      <div class="notice-dialog-container">
        ...
        <div class="notice-dialog-cont">
          ...
          <p><span>4.</span>{{$t('otc_ad.otc_trade_notice4')}}
            <!--收款时请仔细核实，买方的付款账户实名信息与平台实名信息是否一致。卖方收到的资金非买方本人转账，请卖方原路退回。--></p>
          ...
          <p><span>8.</span>{{$t('otc_ad.otc_trade_notice8')}}<!--更详细的交易规范请查看：--><a
            href="https://gcox.kf5.com/hc/kb/article/1225857/" target="_blank">{{$t('otc_ad.otc_trade_specification')}}
            <!--OTC交易规范--></a></p>
        </div>
      </div>
    </div>
    <ul class="list">
      <li class="list-title" v-if="!loading && datas.length>0">
        ...
        <span class="price">{{$t(params.ad_type==2?'otc_exchange.otc_exchange_ask':'otc_exchange.otc_exchange_bid')}}
          <!--单价--></span>
      ...
      <li class="list-item" v-for="data in datas" :key="data.ad_id">
        ...
        <span class="operation">
                  <a href="javascript:;"
                     :class="[sellType ? 'sell' : 'buy', getUserInfo.username === data.username ? 'disabled' : '']"
                     @click="getUserInfo.username === data.username ? false : sell(data)">{{tradeParams.tradeTxt}}</a>
              </span>
      </li>
...
<style scoped>
  ...
  .pendorder-title span.quantity i {
    padding-right: 8px;
    font-size: 25px;
    color: #408BFF;
  }
  ...
  .pendorder-title span.freeze em {
    max-width: 123px;
    word-wrap: break-word;
  }

  .pendorder-title span.transfer {
    width: 55px;
  }
  ...
  .pendorder-title span.operate .notice {
    margin-right: 24px;
    color: #659DFF;
  }

  .pendorder-title span.operate .notice:hover {
    color: #3368c5;
  }

  .pendorder-title span.operate .werben {
    width: 84px;
    height: 35px;
    line-height: 35px;
    font-size: 14px;
  ...
  .pendorder-title span.operate .werben:hover {
    color: #fff;
  ...
  .notice-dialog-cont p a {
    color: #00a0e9;
    text-decoration: underline;
  }

  .notice-dialog-cont p a:hover {
    color: #2565c5;
  }
  ...
  .list-title span.trust {
    width: 125px;
  }

  .list-title span.paytype {
    width: 150px;
  }

  .list-title span.limit {
    width: 140px;
  }

  .list-title span.price {
    width: 110px;
  }

  .list-title span.lave {
    width: 120px;
  }

  .list-title span.name em {
    font-size: 12px;
  }

  .list-title span.name .tips {
    display: inline-block;
    margin-left: 6px;
  ...
  .list-title span.name .tips-container {
    display: flex;
    justify-content: center;
    position: relative;
    z-index: 1;
  ...
  .list-title span.name .tips-icon {
    width: 14px;
    height: 14px;
    font-weight: bold;
    font-size: 12px;
  ...
  .list-title span.name .tips-text {
    display: none;
    position: absolute;
    top: -30px;
    height: 24px;
  ...
  .list-title span.name .tips-text:before {
    content: "";
    position: absolute;
    bottom: -6px;
    left: 50%;
  ...
  .list-title span.name .tips-icon:hover + .tips-text {
    display: block;
  }
  ...
  .list-item span.trust {
    width: 110px;
  }

  .list-item span.paytype {
    width: 150px;
  }

  .list-item span.limit {
    width: 140px;
  }
  ...
  .list-item span p:first-of-type {
    font-size: 16px;
    color: #ffffff;
    white-space: nowrap;
  ...
  .list-item span p:last-of-type {
    font-size: 12px;
    color: #ccc;
  }

  .list-item span.name img {
    width: 40px;
    height: 40px;
    border: 2px solid #434146;
    border-radius: 50%;
    box-sizing: border-box;
  }

  .list-item span.name em {
    width: 88px;
    font-size: 18px;
    color: #ffffff;
    white-space: nowrap;
  ...
  .list-item span.paytype em {
    margin-left: 10px;
    font-size: 25px;
    color: red;
  }

  .list-item span.paytype em:first-of-type {
    margin-left: 0;
  }

  .list-item span.paytype em.icon-paypal {
    color: #2a74d3;
  }

  .list-item span.paytype em.icon-alipay {
    color: #1cc5e4;
  }

  .list-item span.paytype em.icon-wechat {
    color: #69c03e;
  }

  .list-item span.paytype em.icon-bank {
    color: #0D66EF;
  }

  .list-item span.operation a {
    display: block;
    width: 80px;
    height: 35px;
    line-height: 35px;
  ...
  .list-item span.operation a.buy {
    background-color: #23CD09;
  }

  .list-item span.operation a.buy:hover {
    background-color: #1da907;
  }

  .list-item span.operation a.sell {
    background-color: #F34246;
    /*color: #ffffff !important;*/
  }

  .list-item span.operation a.sell:hover {
    background-color: #ca383b;
  }

  .list-item span.operation a.disabled {
    color: #686a77;
    border-color: #686a77;
    cursor: not-allowed;
  }
  ...
  .list li.list-nodata .nd-text {
    height: 40px;
    line-height: 20px;
    color: #8b94a9;
  }

  .list li.list-loading {
    display: block;
  }
</style>
