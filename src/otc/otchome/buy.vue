  ...
  <div class="buy">
    ...
    <div class="buy-cont">
      ...
      <div class="btn">
        ...
        <span class="cimfir" :class="{disabled:locked}" @click="createOrder">{{$t('otc_ad.otc_ad_confirm')}}
          <!--确认--></span>
      </div>
    </div>
  </div>
...
<style scoped>
  ...
  .buy-cont p input.error {
    border-color: #e53f3f;
  }

  .buy-cont p em.asterisk {
    position: relative;
    top: 5px;
    line-height: 0;
    font-size: 18px;
  ...
  .buy-cont p .label-left {
    min-width: 60px;
  }

  .buy-cont p .trade-limit {
    padding-top: 5px;
    padding-bottom: 5px;
    line-height: 0;
  }

  .buy-cont p .trade-limit em {
    display: inline-block;
    line-height: 20px;
    vertical-align: top;
  }

  .buy-cont p .trade-limit em:first-of-type {
    max-width: 168px;
    text-align: right;
  }
