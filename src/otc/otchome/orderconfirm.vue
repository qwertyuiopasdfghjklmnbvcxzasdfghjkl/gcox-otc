<template>
  <div class="order-confirm">
    <div class="title">{{$t(item.trade_type === 1 ? 'exchange.exchange_buy' : 'otc_exchange.otc_exchange_sell')}}
      {{item.symbol}}
    </div>
    <ul class="order-detail">
      ...
      <li class="order-row">
        ...
        <span class="order-pay">
                  <em v-for="data in paylist" :key="data.id" :class="data.css"
                      v-if="item.matchPayType.indexOf(data.id) !== -1"></em>
              </span>
      </li>
    </ul>
  </div>
...
<style scoped>
  ...
  .order-row /deep/ span:last-of-type {
    padding-right: 0;
  }
