<template>
  <div class="delegate">
    <div class="record entrust-current" v-if="show === 'now'">
      <div class="filtrate">
        <div class="time">
        ...
        <div class="button">
          ...
          <button class="reset" @click="clearSearchCur('cur')">{{$t('trade_record.trade_record_reset')}}
            <!--重置--></button>
        </div>
        <div class="operation allrepeal" :class="{disabled: currentEntrust.length === 0}">
          <a href="javascript:;" @click="currentEntrust.length === 0 ? false : cancelOrder('all')">{{$t('trade_record.trade_record_repeal')}}
            <!--全部撤销--></a>
          <i class="icon-repeal"></i>
        </div>
      </div>
      <!--<h3>{{$t('trade_record.current_entrust')}}&lt;!&ndash;当前委托&ndash;&gt;</h3>-->
      ...
      <ul v-if="!currentLoading && currentEntrust.length > 0">
        <li class="list" v-for="(item, index) in currentEntrust" :key="item.id">
          ...
          <span class="operation"><a href="javascript:;" @click="cancelOrder(item.orderBookId, index)">{{$t('exchange.exchange_revoked')}}
            <!--撤销--></a></span>
        </li>
      </ul>
      <page v-if="!currentLoading && currentEntrust.length > 0" :pageIndex="curParams.current"
            :pageSize="curParams.limit" :total="curTotal" @changePageIndex="curPageChange"/>
    ...
    <div class="record entrust-history" v-if="show === 'history'">
      <div class="filtrate">
        <div class="time">
          <label>{{$t('trade_record.trade_record_time')}}：<!--时间--></label>
        ...
        <div class="button">
          ...
          <button class="reset" @click="clearSearchCur('his')">{{$t('trade_record.trade_record_reset')}}
            <!--重置--></button>
        </div>
        <div class="operation hide" :class="{disabled: historyEntrust.length === 0 && !historyParams.hideCancelled}"
             @click="historyEntrust.length === 0 && !historyParams.hideCancelled ? false : hisHideCancellFun(historyParams.hideCancelled)">
          <a href="javascript:;">{{$t('trade_record.trade_record_hide')}}<!--隐藏已取消记录--></a>
          <i class="icon-checkbox"
             :class="[historyParams.hideCancelled === 0 ? 'icon-checkbox-unchecked':'icon-checkbox-checked']"></i>
        </div>
        <div class="operation export" :class="{disabled: historyEntrust.length === 0}">
          <a href="javascript:;" @click="historyEntrust.length === 0 ? false : onExportRecord()">{{$t('trade_record.trade_record_export')}}
            <!--导出报表--></a>
          <i class="icon-reports"></i>
        </div>
      </div>
      <!--<h3>{{$t('trade_record.history_entrust')}}&lt;!&ndash;历史委托记录&ndash;&gt;</h3>-->
      ...
      <page v-if="!historyLoading && historyEntrust.length > 0" :pageIndex="historyParams.current"
            :pageSize="historyParams.limit" :total="historyTotal" @changePageIndex="hisPageChange"/>
...
<style scoped>
  ...
  .delegate /deep/ .filtrate .operation {
    margin-right: 0;
    cursor: pointer;
  }

  .delegate /deep/ .filtrate .allrepeal,
  .delegate /deep/ .filtrate .export {
    margin-left: auto;
  }

  .delegate /deep/ .filtrate label {
    font-size: 14px;
    color: #979799;
  }

  .delegate /deep/ .filtrate input {
    height: 22px;
  ...
  .delegate /deep/ .filtrate select {
    ...
    font-size: 12px;
    color: #666;
    background: url(../../../assets/images/icon_arrowdown.png) no-repeat right 4px center;
    border-bottom: 1px solid #312e45;
    cursor: pointer;
    border-radius: 3px;
  }

  .delegate /deep/ .filtrate button {
    float: left;
    min-width: 38px;
    height: 24px;
    font-size: 12px;
  ...
  .delegate /deep/ .filtrate .time /deep/ input {
    width: 100px;
    padding-right: 24px;
    background: url(../../../assets/images/icon_calendar.png) no-repeat right 4px center;
    cursor: pointer;
  }

  .delegate /deep/ .filtrate .market input {
    width: 50px;
  }

  .delegate /deep/ .filtrate .button button {
    min-width: 60px;
  }

  .delegate /deep/ .filtrate .button .search {
    margin-right: 14px;
    color: #fff;
    background-color: #00B5FF;
  }

  .delegate /deep/ .filtrate .button .search:hover {
    background-color: #1a64d4;
  }

  .delegate /deep/ .filtrate .button .reset {
    color: #999;
    border: 1px solid #ccc;
  }

  .delegate /deep/ .filtrate .button .reset:hover {
    color: #666;
    border-color: #bbb;
  }
  ...
  .delegate /deep/ .filtrate .export i {
    font-size: 14px;
  }

  .delegate /deep/ .filtrate .operation:hover a,
  .delegate /deep/ .filtrate .operation:hover i {
    color: #1a64d4;
  }
  ...
  .delegate /deep/ .filtrate .disabled i {
    color: #999;
    cursor: not-allowed;
  }

  .delegate /deep/ .filtrate .disabled:hover a,
  .delegate /deep/ .filtrate .disabled:hover i {
    color: #999;
  }
  ...
  .delegate /deep/ .record ul li span {
  ...
  .delegate /deep/ .entrust-current ul li span.market {
    width: 100px;
  }

  .delegate /deep/ .entrust-current ul li span.type {
    width: 50px;
  }

  .delegate /deep/ .entrust-current ul li span.price {
    width: 140px;
  }

  .delegate /deep/ .entrust-current ul li span.tradeVolume {
    width: 150px;
  }

  .delegate /deep/ .entrust-current ul li span.volume {
    width: 140px;
  }

  .delegate /deep/ .entrust-current ul li span.sum {
    width: 140px;
  }
  ...
  .delegate /deep/ .entrust-history ul li span.market {
    width: 80px;
  }

  .delegate /deep/ .entrust-history ul li span.type {
    width: 50px;
  }

  .delegate /deep/ .entrust-history ul li span.price {
    width: 100px;
  }

  .delegate /deep/ .entrust-history ul li span.avgPrice {
    width: 120px;
  }

  .delegate /deep/ .entrust-history ul li span.tradeVolume {
    width: 130px;
  }

  .delegate /deep/ .entrust-history ul li span.volume {
    width: 120px;
  }

  .delegate /deep/ .entrust-history ul li span.tradeSum {
    width: 120px;
  }

  .delegate /deep/ .entrust-history ul li span.status {
    width: 60px;
  }

  .delegate /deep/ .record ul li.list span.buy {
    color: #23CD09;
  }

  .delegate /deep/ .record ul li.list span.sell {
    color: #F34246;
  }
  ...
  .delegate /deep/ .record .nodata {
    text-align: center;
  }

  .delegate /deep/ .record .nodata .nodata-icon {
    height: 80px;
    line-height: 80px;
    font-size: 40px;
    color: #A1A1A1;
  ...
  .delegate /deep/ .record .nodata .nodata-text {
    height: 40px;
    line-height: 20px;
    color: #A1A1A1;
  }
