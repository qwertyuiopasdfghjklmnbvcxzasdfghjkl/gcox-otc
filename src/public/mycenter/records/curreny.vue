    ...
    <div class="record">
      <div class="filtrate">
        ...
        <div class="operation export" :class="{disabled: coinsEntrust.length === 0}">
          <a href="javascript:;" @click="coinsEntrust.length === 0 ? false : onExportRecord()">{{$t('trade_record.trade_record_export')}}
            <!--导出报表--></a>
          <i class="icon-reports"></i>
        </div>
      </div>
      <!--<h3>{{$t('trade_record.exchange_record')}}&lt;!&ndash;币币成交记录&ndash;&gt;</h3>-->
      ...
      <page v-if="!coinsLoading && coinsEntrust.length > 0" :pageIndex="coinsParams.current"
            :pageSize="coinsParams.limit" :total="coinsTotal" @changePageIndex="coinsPageChange"/>
...
<style scoped>
  ...
  .curreny /deep/ .filtrate .operation {
    margin-right: 0;
    cursor: pointer;
  }

  .curreny /deep/ .filtrate .allrepeal,
  .curreny /deep/ .filtrate .export {
    margin-left: auto;
  }

  .curreny /deep/ .filtrate label {
    font-size: 14px;
    color: #979799;
  }

  .curreny /deep/ .filtrate input {
    height: 22px;
  ...
  .curreny /deep/ .filtrate .time /deep/ input {
    width: 100px;
    padding-right: 24px;
  ...
  .curreny /deep/ .filtrate .market input {
    width: 50px;
  }

  .curreny /deep/ .filtrate .button button {
    min-width: 60px;
  }

  .curreny /deep/ .filtrate .button .search {
    margin-right: 14px;
    color: #fff;
    background-color: #00B5FF;
  }

  .curreny /deep/ .filtrate .button .search:hover {
    background-color: #1a64d4;
  }

  .curreny /deep/ .filtrate .button .reset {
    color: #999;
    border: 1px solid #ccc;
  }

  .curreny /deep/ .filtrate .button .reset:hover {
    color: #666;
    border-color: #bbb;
  }
  ...
  .curreny /deep/ .filtrate .export i {
    font-size: 14px;
  }

  .curreny /deep/ .filtrate .operation:hover a,
  .curreny /deep/ .filtrate .operation:hover i {
    color: #1a64d4;
  }

  .curreny /deep/ .filtrate .hide a, .curreny /deep/ .filtrate .hide:hover a {
    color: #333;
  }

  .curreny /deep/ .filtrate .disabled a,
  .curreny /deep/ .filtrate .disabled i {
    color: #999;
    cursor: not-allowed;
  }

  .curreny /deep/ .filtrate .disabled:hover a,
  .curreny /deep/ .filtrate .disabled:hover i {
    color: #999;
  }
  ...
  .curreny /deep/ .record ul li span {
  ...
  .curreny /deep/ .record ul li span.market {
    width: 100px;
  }

  .curreny /deep/ .record ul li span.type {
    width: 60px;
  }

  .curreny /deep/ .record ul li span.avgPrice {
    width: 120px;
  }

  .curreny /deep/ .record ul li span.tradeVolume {
    width: 120px;
  }

  .curreny /deep/ .record ul li span.tradeSum {
    width: 120px;
  }

  /*.curreny /deep/ .record ul li span.charge{width: 250px;}*/
  .curreny /deep/ .record ul li.list span.buy {
    color: #23CD09;
  }

  .curreny /deep/ .record ul li.list span.sell {
    color: #F34246;
  }

  .curreny /deep/ .record .nodata {
    text-align: center;
  }

  .curreny /deep/ .record .nodata .nodata-icon {
    height: 80px;
    line-height: 80px;
    font-size: 40px;
    color: #A1A1A1;
  ...
  .curreny /deep/ .record .nodata .nodata-text {
    height: 40px;
    line-height: 20px;
    color: #A1A1A1;
  }
